...
class BatchedInput:
    ...
    txt_instruction: list[str] = None  # text instruction
...
def make_batch_input(batch: Union[Dict, List, Tuple]):
    if isinstance(batch, dict):
        return BatchedInput(
            ...
            # text instruction
            txt_instruction=batch['info.json'] if 'info.json' in batch else None,
        )
...
class MaskGitLFQTrainer:
    def __init__(self,
        ...
        if cfg.maskgit_cfg.use_aux_loss:
            self.vae_decoder.requires_grad_(False)
        self.quantizer.requires_grad_(False)
    
        # maskgit model
        self.maskgit_cfg = cfg.maskgit_cfg
        self.maskgit = MaskBit(**cfg.maskgit_cfg.model_cfg)
    ...
    def make_simple_vq_model_to_sample(self, token_size: list[int]):
        ...
        class SimpleVQSampleModel(torch.nn.Module):
            ...
            def decode_tokens(self, decoded_codes: Tensor):
                # if use factorized tokens, then
                # predicted_indices = combine_factorized_tokens(predicted_indices,
    ...
    def train_step(self, batch: BatchedInput):
        ...
        # mask images
        # masked_codes: [b, l * m, k // m], masks: [b, l * m]
        #* ver. 1: maskbit style, half the bit to be masked by group
        # masked_codes, masks = self.mask_inp_codes(inp_codes)

        with self.accelerator.accumulate() and self.accelerator.autocast():
            # to maskgit
            #* ver. 2 random mask
            ret = self.maskgit.forward_loss_random(inp_codes, (source1_codes, source2_codes), instruction)
        ...
        if self.may_update_progress:
            tbar.update(train_iter_task, total=self.train_cfg.max_train_steps, completed=self.train_n_iter, visible=True,
                        description=f'Train Iter [{self.train_n_iter}/{self.train_cfg.max_train_steps}] - ' + \
                                    f'mask_loss: {ret["loss"].item():.4f}, ' + \
                                    f'correct_tokens: {ret["correct_tokens"].item():.4f}, ' + \
        ...
        if self.train_n_iter % self.log_cfg.log_every == 0:
            self.logger.info(f'[b][Train][/b] global step: {self.train_n_iter} | lr: {self.lr_scheduler_maskgit.get_last_lr()[0]:.4e} | ' + \
                             f'mask_loss: {ret["loss"].item():.4f} | correct_tokens: {ret["correct_tokens"].item():.4f} | ' + \
                             f'aux_loss: {round(aux_loss["loss"].item(), 4) if self.train_cfg.use_auxiliary_loss else "N/A"}')
            self.logger.log_curves({'loss/mask_loss': ret["loss"].item(), 
                                    'acc/correct_tokens': ret["correct_tokens"].item(),
    ...
    def val_step(self, batch: BatchedInput):
        ...
        sampled_model = self.ema_maskgit.ema_model if self.train_cfg.sampled_use_ema else self.maskgit
        
        # sample
        gen_img, sampled_full_tokens = sampled_model.sample(
            ...
            gumbel_temperature=0.5 # self.maskgit_cfg.gumbel_temperature
        )
