...
class CrossScan(torch.autograd.Function):
    ...
    def forward(ctx, x: torch.Tensor):
        B, C, H, W = x.shape
        ctx.shape = (B, C, H, W)
        xs = x.new_empty((B, 4, C, H * W), dtype=x.dtype)
        xs[:, 0] = x.flatten(2, 3)
...
class CrossMerge(torch.autograd.Function):
    ...
    def backward(ctx, x: torch.Tensor):
        ...
        H, W = ctx.shape
        B, C, L = x.shape
        xs = x.new_empty((B, 4, C, L), dtype=x.dtype)
        xs[:, 0] = x
...
def cross_selective_scan(
    ...
    if out_norm_shape in ["v1"]: # (B, C, H, W)
        y = out_norm(y.view(B, -1, H, W).to(dtype)).permute(0, 2, 3, 1) # (B, H, W, C)
    else: # (B, L, C)
        y = y.transpose(dim0=1, dim1=2).contiguous() # (B, L, C)
        y = out_norm(y.to(dtype)).view(B, H, W, -1)
