...
logger = easy_logger(func_name='rwkv_v2')
...
if T_MAX > _TMAX_DEFAULT:
    logger.warning(f"T_MAX is set to {_TMAX_DEFAULT}, which is greater than {_TMAX_DEFAULT}. This may consume a lot of memory")
else:
...
class VRWKV_SpatialMix_V6(nn.Module):
        ...
        if init_mode == "fancy":
            with torch.no_grad():
                ...
                self.time_maa_w1 = nn.Parameter(
                    torch.zeros(TIME_MIX_EXTRA_DIM * 5, self.n_embd).uniform_(-0.01, 0.01)
                )
                self.time_maa_w2 = nn.Parameter(
                    torch.zeros(5, TIME_MIX_EXTRA_DIM, self.n_embd).uniform_(-0.01, 0.01)
                )
    ...
    def jit_func(self, x, patch_resolution):
        ...
        if self.shift_mode == 'q_shift_multihead':
            ...
            xx = self.shift_func(x, patch_resolution[0], patch_resolution[1]) - x
        else:
            xx = x
        ...
        mw, mk, mv, mr, mg = xxx.unbind(dim=0)  # [B, T, C]
