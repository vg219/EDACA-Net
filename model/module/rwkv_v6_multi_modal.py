...
class VRWKV_SpatialMix_wkv5(nn.Module):
    ...
    def cat_seqs_wo_special_tokens(self, img: torch.Tensor, txt: torch.Tensor):
        bs_txt, L_t, C_t = txt.shape
        bs_img, L_i, C_i = img.shape
        assert C_t == C_i, "modalities should have the same channels, but got {} and {}".format(C_t, C_i)
    ...
    def jit_func(self, x, txt=None, resolution=None, mm_tokens: "tuple[torch.Tensor] | None"=None):
        ...
        # 2. scan to 1d sequence
        x = self.scan(x).view(B * self.scan_K, -1, T).transpose(1, 2)  # [b*k, l, c]
        xx = self.scan(xx).view(B * self.scan_K, -1, T).transpose(1, 2)  # [b*k, l, c]
            
        # 3. cat txt and special tokens
        if txt is not None:
            if mm_tokens is not None:
                x = self.cat_special_tokens_to_seqs(x, txt, *mm_tokens)
                xx = self.cat_special_tokens_to_seqs(xx, txt, *mm_tokens)
    ...
    def forward(self, x, txt=None, patch_resolution=None, mm_tokens: "tuple[torch.Tensor] | None"=None):
        assert self.with_txt == (txt is not None), "txt should be provided when with_txt is True"
        
        def _inner_forward(x, txt, patch_resolution, mm_tokens):
            B, C, H, W = x.size()
            r, k, v, g, T = self.jit_func(x, txt, patch_resolution, mm_tokens)
            x = RUN_CUDA_RWKV5_2(*r.shape, self.n_head, r, k, v, w=self.time_decay, u=self.time_faaaa)
            ...
            if not self.with_txt:
                x = rearrange(x, '(b k) (h w) d -> b k d h w', k=self.scan_K, h=H, w=W)
                x = self.merge(x) / self.scan_K  # [b, c, h, w]
                x = x.view(B, C, H * W).transpose(1, 2)
            
            return x
            
        if self.with_cp and x.requires_grad:
            return cp.checkpoint(_inner_forward, x, txt, patch_resolution, mm_tokens, use_reentrant=False)
        else:
            return _inner_forward(x, txt, patch_resolution, mm_tokens)
