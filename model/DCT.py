...
class DCT(BaseModel):
    ...
    def sharpening_train_step(self,lms, lr_hsi, pan, gt, criterion):
        sr = self._forward_implem(lr_hsi, pan)
    ...
    def sharpening_val_step(self,
        ...
        if patch_merge:
            ...
            pred = self._forward_implem(lr_hsi, pan)

        return pred.clip(0, 1)

    def patch_merge_step(self, lr_hsi, pan, *args, **kwargs):

        return self._forward_implem(lr_hsi, pan)
    

if __name__ == "__main__":
    torch.cuda.set_device("cuda:1")
    net = DCT(31, 16, 180).cuda()
    hr = torch.rand(1, 3, 16, 16).cuda()
    lr = torch.rand(1, 31, 1, 1).cuda()
    lms = torch.rand(1, 31, 16, 16).cuda()
    gt = torch.rand(1, 31, 16, 16).cuda()
