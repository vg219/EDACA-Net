...
logger = easy_logger(func_name='FeINFN', level='INFO')
...
class FeINFNet(BaseModel):
    def __init__(self, hsi_dim=9, msi_dim=8,feat_dim=128, guide_dim=128, spa_edsr_num=4, spe_edsr_num=4, mlp_dim=[256, 128], NIR_dim=10, d_model=2,
        ...
        self.spatial_encoder = make_edsr_baseline(n_resblocks=spa_edsr_num, n_feats=self.guide_dim, n_colors=hsi_dim)
        self.spectral_encoder = make_edsr_baseline(n_resblocks=spe_edsr_num, n_feats=self.feat_dim, n_colors=msi_dim)
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(  # 输入的图片 （16，64，64）
                in_channels=feat_dim,
                out_channels=feat_dim,  # 经过一个卷积层之后 （16,64,64）
                kernel_size=3,
                stride=1,
                padding=1
            ),
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2)  # 经过池化层处理，维度为（16,32,32）
        )

        self.conv2 = nn.Sequential(
            nn.Conv2d(  # 输入（16,32,32）
                in_channels=feat_dim,
            ...
            nn.ReLU(),
            nn.MaxPool2d(kernel_size=2)  # 输出（16,16,16）
        )
    ...
    def _forward_implem(self, pan, lms, ms):
        # HR_MSI Bx3x64x64
        # lms Bx31x64x64
        # LR_HSI Bx31x16x16
        ...
        feat = torch.cat([pan, lms], dim=1)
        hr_spa = self.spatial_encoder(feat)  # Bx128xHxW
        lr_spe = self.spectral_encoder(ms)  # Bx128xhxw The feature map of LR-HSI
        hr_spa1 = self.conv1(hr_spa)
        hr_spa2 = self.conv1(hr_spa1)
        lr_spe1 = torch.cat([lr_spe,hr_spa2],dim=1)
        freq_feature = self.freq_query(lr_spe1, coord, hr_spa)
    ...
    def sharpening_train_step(self,lms, ms, pan, gt, txt, criterion):
    ...
    def sharpening_val_step(self,
        ...
        if self.patch_merge:
            ...
            pred = _patch_merge_model.forward_chop(lms,ms,pan)[0]
        else:
            pred = self._forward_implem(pan,lms,ms)
        return pred.clip(0, 1)

    def patch_merge_step(self,lms, ms, pan,txt, *args, **kwargs):
...
if __name__ == '__main__':
    ...
    net = FeINFNet(hsi_dim=9, msi_dim=8,feat_dim=128, guide_dim=128, spa_edsr_num=4, spe_edsr_num=4, mlp_dim=[256, 128], NIR_dim=10).cuda()

    B, C, H, W = 1, 8, 64, 64
    scale = 4

    pan = torch.randn([B, 1, H, W]).cuda()
    lms = torch.randn([B, C, H, W]).cuda()
    ms = torch.randn([B, C, H // scale, W // scale]).cuda()
    gt = torch.randn(1, 8, H, W).cuda()
    criterion = torch.nn.L1Loss()
    txt = None
    net.forward = net._forward_implem
    output,loss = net.sharpening_train_step(lms, ms, pan, gt, txt, criterion)
    print(output.shape)
    print(flop_count_table(FlopCountAnalysis(net, (pan, lms, ms))))
