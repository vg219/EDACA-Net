...
class MHIIF_(BaseModel):
    ...
    def query(self, feat, coord, hr_guide):
        ...
        inp_grad = self.grad_gt(pred.permute(0, 2, 1).view(b, -1, H, W)).permute(0, 2, 3, 1).reshape(b, H*W, -1)
                # inp_grad = self.grad(pred.permute(0, 2, 1).view(b, -1, H, W)).permute(0, 2, 3, 1).reshape(b, H*W, -1)
        pred_grad = self.imnet_grad(inp_grad.view(B * N, -1)).view(B, N, -1)


        preds = torch.stack(pred, dim=-1)  # [B, N, C, kk]
        preds_grad = torch.stack(pred_grad, dim=-1)  # [B, N, C, kk]
...
if __name__ == '__main__':
    ...
    # print(output.shape)
    model.forward = model._forward_implem_v3
