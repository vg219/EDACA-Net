...
class LFQBert(torch.nn.Module):
    ...
    def forward(
        ...
        # indices to bits to the model
        # shape as (bs, n, (m c)), m x c = k
        ...
        # k -> hidden_dim, shape as (bs, n, m x c) -> (bs, n, hidden_dim)
        ...
        # rearrange to (bs, n, m, 2 ** (k // m)) by splitting k codebook
        logits = rearrange(self.prediction_layer(x), "b n (m c) -> b n m c",
                           c=self.effective_codebook_size, m=self.splits)        
...
if __name__ == "__main__":
    ...
    def get_factorized_ids(codes: Tensor, code_book_dim: int=18, split_m: int=2):
        # gt codes: (bs, l, k)
        # return ids: (bs, l * m)
        gt_codes_factorized = rearrange(codes, 'b n (m c) -> b n m c', m=split_m)
        codebook_dim_factorized = code_book_dim // split_m
        gt_indices_lst = []
        for i in range(split_m):
            part_codes = gt_codes_factorized[:, :, i]  # [b, n, c]       
            part_codes = (part_codes + 1) / 2  # to [0, 1]
            indices = 2 ** torch.arange(0, codebook_dim_factorized, 1, 
                                        dtype=torch.long, device=codes.device)
            indices = (part_codes * indices).sum(-1)
            gt_indices_lst.append(indices.long())
        return torch.cat(gt_indices_lst, dim=1)
    
    # mlm_loss = MLMLoss(loss_weight_unmasked_token=0.)
    gt_ids = get_factorized_ids(lfq_codes, code_book_dim=18, split_m=factorized_n)
    # mlm_loss, loss_dict = mlm_loss(logits, gt_ids, masks)
    
    masks = masks.bool()
    logits_for_loss = rearrange(logits, 'b ... logits -> b (...) logits')[masks]
    labels_for_loss = gt_ids[masks]
    
    # cross_entropy([bs, l * m, 2 ** (k // m)], [bs, l * m])
