...
from model.lfq_vae.gan_loss.patchgan_discriminator import NLayerDiscriminator, weights_init
...
class VQLPIPSWithDiscriminator(nn.Module):
    def __init__(self, disc_start=0, codebook_weight=1.0, pixelloss_weight=1.0,
        ...
        if loss_ssim:
            ...
            print(f"SSIM Loss is used in VAE losses")
    ...
    def forward(
        ...
        if optimizer_idx == 0:
            ...
            if self.with_ssim:
                ssim_loss = self.ssim_loss((inputs + 1) / 2, (reconstructions + 1) / 2) * 0.1
            else:
            ...
            loss = nll_loss + ssim_loss + g_loss + scale_codebook_loss + loss_break.commitment * self.commit_weight
            
            if disc_factor == 0:
                log = {
                    "{}/total_loss".format(split): loss.clone().detach(),
                    "{}/per_sample_entropy".format(split): loss_break.per_sample_entropy.detach(),
                    "{}/codebook_entropy".format(split): loss_break.batch_entropy.detach(),
