...
LossBreakdown = namedtuple('LossBreakdown', ['per_sample_entropy', 'batch_entropy', 'commitment', 'avg_probs'])
...
class LFQ(Module):
    ...
    def indices_to_codes(self, indices):
        codes = self.indices_to_bits(indices) * 2 - 1
        codes = rearrange(codes, 'bs ... d -> bs d ...')
        return codes

    def get_codebook_entry(self, x, bhwc, order, is_neg_1_to_1=True): #0610
    ...
    def forward(
        ...
        if self.token_factorization:
            ...
            indices = unpack_one(indices, ps, 'b * c').squeeze(-1)

        ret = Return(quantized, indices, entropy_aux_loss)
