...
ReturnInfo = namedtuple(
    "ReturnInfo", ["H", "used_codes", "indices", "group_indices", "avg_prob", "logit_laplace_loss"],
...
if __name__ == "__main__":
    ...
    # dtype = torch.float32
    # zq = torch.zeros((4, 32, K), dtype=dtype, device='cuda') * 2 - 1
    # basis = (2 ** torch.arange(K - 1, -1, -1)).to(dtype).cuda()
    # zq.requires_grad = True
    # h = codebook_entropy(zq, basis, K)
    # h.backward()
    # print(zq.grad, zq)

    bsq = BinarySphericalQuantizer(embed_dim=18, l2_norm=False)
    z = torch.randn(1, 18, 16, 16)
    zq, loss, info = bsq(z)
    print(zq.shape, loss, info.indices.shape)
