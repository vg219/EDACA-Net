...
def maybe_distributed_mean(t, dist_entropy):
    if not is_distributed() or not dist_entropy:
        return t
    ...
    def forward(
        ...
        with quantization_context():
            ...
            if self.training:
                ...
                avg_prob = maybe_distributed_mean(avg_prob, self.dist_entropy)
        ...
        # complete aux loss
        # entropy loss and commitment loss
...
if __name__ == "__main__":
    lfq = LFQ(dim=18, channel_first=True)
    x = torch.randn(1, 18, 16, 16)
    (quantized, indices, aux_loss), loss_breakdown = lfq(x, inv_temperature=100., return_loss_breakdown=True)
    print(quantized.shape)
    print(aux_loss)
    print(indices.shape)
    
    print(loss_breakdown.per_sample_entropy)
    print(loss_breakdown.batch_entropy)
    print(loss_breakdown.commitment)
