...
from model.module.fe_block import (
    make_edsr_baseline,
    make_coord,
    ComplexGaborLayer,
    PositionalEmbedding,
...
class PositionalEncoder(nn.Module):
    ...
    def __init__(self, in_dim=2, out_dim=64):
        ...
        self.proj = nn.Sequential(
            nn.Linear(8 * len(self.freq_bands), out_dim), nn.GELU()
        )
    ...
    def forward(self, coord):
        ...
        # 计算基础编码
        sins = torch.sin(
            scaled_coord[..., None] * self.freq_bands.to(coord.device)
        )  # [B, N, 2, 16]
        coss = torch.cos(
            scaled_coord[..., None] * self.freq_bands.to(coord.device)
        )  # [B, N, 2, 16]
...
@register_model("MHIIF_g")
class MHIIF_g(BaseModel):
    def __init__(
        self,
        hsi_dim=31,
        msi_dim=3,
        ...
        scale=4,
        patch_merge=True,
        sample_type: Literal["simple_loop", "vmap"] = "simple_loop",
    ):
        super().__init__()
        ...
        imnet_grad_in_dim = 3 * self.feat_dim + 2 * self.guide_dim + 2
        self.imnet = MLP(imnet_in_dim, out_dim=hsi_dim + 1, hidden_list=self.mlp_dim)
        self.imnet_grad = MLP(
            imnet_grad_in_dim, out_dim=hsi_dim + 2, hidden_list=self.mlp_dim
        )
    ...
    def _neighbor_sample(
        ...
        B, N, C = hr_guide_feat.shape
        # N = H * W
        device = feat.device

        def _simple_for_loop_sample():
            preds = []
            preds_grad = []
            for vx in [-1, 1]:
                for vy in [-1, 1]:
                    ...
                    q_feat = (
                        F.grid_sample(
                            feat,
                            offset_coord.flip(-1).unsqueeze(
                                1
                            ),  # grid_sample需要[x,y]顺序
                            mode="nearest",
                            align_corners=False,
                        )
                        .squeeze(2)
                    ...
                    pos_embed = self.position_encoder(
                        rel_coord.view(B * N, 2)
                    )  # [B*N, pe_dim]
                    pos_embed = pos_embed.view(B, N, -1)  # [B, N, 4, pe_dim]
                    input = torch.cat(
                        [
                            q_feat,  # 原始特征 [B,N,C]
                            hr_guide_feat,  # 高分辨率引导 [B,N,4,C_guide]
                            rel_coord,  # 相对坐标差 [B, N, 2]
                        ],
                        dim=-1,
                    )  # 最终维度: C + 2C + pe_dim + C_guide + 2
                    # 构建最终输入特征
                    grad_input = torch.cat(
                        [
                            q_grad,  # 梯度特征 [B,N,2C]
                            hr_guide_feat_grad,  # 高分辨率引导 [B,N,2C_guide]
                            pos_embed,  # 位置编码 [B,N,pe_dim]
                            rel_coord,  # 相对坐标差 [B, N, 2]
                        ],
                        dim=-1,
                    )  # 最终维度: C + 2C + pe_dim + C_guide + 2
                    ...
                    pred = self.imnet(input.view(B * N, -1)).view(B, N, -1)
                    pred_grad = self.imnet_grad(grad_input.view(B * N, -1)).view(
                        B, N, -1
                    )
                    # 保存所有采样结果
                    preds.append(pred)
            ...
            preds = torch.stack(preds, dim=-1)  # [B, N, 4, C]
            preds_grad = torch.stack(preds_grad, dim=-1)  # [B, N, 4, 2C]

            return preds, preds_grad

        def _vmap_sample():
            # Prepare inputs for vmap
            vx_list = torch.tensor([-1, 1, -1, 1], device=device, dtype=torch.float32)
            vy_list = torch.tensor([-1, -1, 1, 1], device=device, dtype=torch.float32)

            # Define the function to be vmapped
            # Pass necessary tensors explicitly for clarity and potentially better vmap handling
            def process_offset(vx, vy, base_coord, # Pass coord explicitly
                               inner_feat, inner_grad_feat, inner_feat_coord,
                               inner_hr_guide_feat, inner_hr_guide_feat_grad):

                # --- Start Modification ---
                # Calculate offsets functionally, not inplace
                offset_x = base_coord[..., 0] + vx * rx * 0.5
                offset_y = base_coord[..., 1] + vy * ry * 0.5
                # Stack to create the offset coordinates
                offset_coord_new = torch.stack([offset_x, offset_y], dim=-1) # Shape [B, N, 2]

                # Use the new offset_coord_new for grid sampling
                offset_coord_grid = offset_coord_new.unsqueeze(1) # [B, 1, N, 2]
                offset_coord_grid = torch.clamp(offset_coord_grid, -1.0, 1.0)
                # --- End Modification ---


                # Sample features and coordinates (use inner_* variables passed)
                q_feat = F.grid_sample(inner_feat, offset_coord_grid, mode='nearest', align_corners=False, padding_mode='border').squeeze(2).permute(0, 2, 1)
                q_grad = F.grid_sample(inner_grad_feat, offset_coord_grid, mode='nearest', align_corners=False, padding_mode='border').squeeze(2).permute(0, 2, 1)
                q_coord = F.grid_sample(inner_feat_coord, offset_coord_grid, mode='nearest', align_corners=False, padding_mode='border').squeeze(2).permute(0, 2, 1)

                # Calculate relative coordinates and positional encoding
                # Use base_coord here as it's the original query coordinate
                rel_coord = base_coord - q_coord
                rel_coord_scaled = rel_coord.clone()
                rel_coord_scaled[..., 0] /= (rx + 1e-8)
                rel_coord_scaled[..., 1] /= (ry + 1e-8)
                # Ensure position encoder handles potential leading batch dim from vmap correctly
                # Flattening usually handles this: view(-1, 2)
                pos_embed = self.position_encoder(rel_coord_scaled.reshape(-1, 2)).view(B, N, -1) # Assume B=1 or adjust if B>1

                # Prepare MLP inputs (use inner_hr_guide features)
                inp = torch.cat([q_feat, inner_hr_guide_feat, rel_coord], dim=-1)
                grad_inp = torch.cat([q_grad, inner_hr_guide_feat_grad, pos_embed, rel_coord], dim=-1)
                # if isinstance(inp, torch.Tensor):
                #     inp = inp.view(-1, inp.shape[-1]).contiguous()
                # else:
                #     inp = torch.cat(inp, dim=-1).view(-1, inp[0].shape[-1]).contiguous()
                # if isinstance(grad_inp, torch.Tensor):
                #     grad_inp = grad_inp.view(-1, grad_inp.shape[-1]).contiguous()
                # else:
                #     grad_inp = torch.cat(grad_inp, dim=-1).view(-1, grad_inp[0].shape[-1]).contiguous()
                # Predict coefficients
                # Ensure MLPs handle potential leading batch dim correctly (view(-1, D) usually works)
                pred = self.imnet(inp.view(B * N, -1)).view(B, N, -1)
                pred_grad = self.imnet_grad(grad_inp.view(B * N, -1)).view(B, N, -1)

                return pred, pred_grad

            # Apply vmap over vx_list and vy_list
            # Explicitly pass other arguments with in_dims=None
            preds_v, preds_grad_v = torch.vmap(process_offset,
                                               in_dims=(0, 0, None, None, None, None, None, None) # Match args
                                              )(vx_list, vy_list,
                                                coord, feat, grad_feat, feat_coord,
                                                hr_guide_feat, hr_guide_feat_grad) # Pass all args

            # Output shapes from vmap will be [4, B, N, hsi_dim+1/hsi_dim+2]
            # Permute to match the loop output: [B, N, hsi_dim+1/hsi_dim+2, 4]
            preds_v = preds_v.permute(1, 2, 3, 0)
            preds_grad_v = preds_grad_v.permute(1, 2, 3, 0)

            return preds_v, preds_grad_v

        if self._sample_type == "simple_loop":
            return _simple_for_loop_sample()
        else:
            return _vmap_sample()
    ...
    def hermite_query(self, feat, coord, hr_guide):
        ...
        grad_hr = self.grad_extractor(hr_guide)  # [B,2_,H,W]
        # 高分辨率引导特征采样
        hr_guide_feat = F.grid_sample(
            hr_guide, coord.flip(-1).unsqueeze(1), mode="nearest", align_corners=False
        )[:, :, 0, :].permute(0, 2, 1)  # [B, N, C_guide]

        hr_guide_feat_grad = F.grid_sample(
            grad_hr, coord.flip(-1).unsqueeze(1), mode="nearest", align_corners=False
        )[:, :, 0, :].permute(0, 2, 1)  # [B, N, 2C_guide]
        ...
        # =================== 第四步： ====
        preds, preds_grad = self._neighbor_sample(
            coord,
            rx,
            ry,
            h,
        ...
        # ================== 第五步：Hermite插值计算 ==================
        # 分解预测系数
        value_weights = torch.softmax(preds[:, :, -1, :], dim=-1)  # [B,N,4]
        grad_weights = torch.sigmoid(preds_grad[:, :, 0, :])  # [B,N,4]
        blend_factors = preds_grad[:, :, -1, :]  # [B,N,4]
        pred = preds[:, :, 0:-1, :]
        pred_grad = preds_grad[:, :, 1:-1, :]
        # 计算各邻域贡献
        value_terms = (
            (pred * value_weights.unsqueeze(-2)).sum(-1, keepdim=True).squeeze(-1)
        )  # [B,N,C]
        grad_terms = (
            (pred_grad * grad_weights.unsqueeze(-2)).sum(-1, keepdim=True).squeeze(-1)
        )  # [B,N,C]
...
if __name__ == "__main__":
    ...
    model = MHIIF_g(31, 3, 64, 64, sample_type="vmap").cuda()
