...
def window_reverse_1d(windows: torch.Tensor, window_size: int):
    assert windows.shape[1] % window_size == 0, "sequence length must be divisible by window size"
    
    # reverse
    # (B N) L/N C -> B L C
    x = rearrange(windows, '(b n) l c -> b (l n) c', n=window_size).contiguous()
    ...
    def __init__(self,
        ...
        self.feat_convs = nn.Sequential(
            nn.Conv2d(n_embd, n_embd, kernel_size=3, padding=1,
                      groups=n_embd, bias=False, padding_mode='reflect'),
            nn.ReLU(),
            nn.Conv2d(n_embd, n_embd, 1, bias=False),
        )
        
        # modalities encoder
        self.modal_convs = nn.Conv2d(modal_chan, n_embd, kernel_size=3, padding=1, bias=False, padding_mode='reflect')
        ...
        if mask_chan is not None:
            ...
            self.mask_convs = nn.Sequential(
                ...
                nn.Conv2d(n_embd // mask_ratio, n_embd, kernel_size=3, padding=1,
                          groups=n_embd // mask_ratio, bias=False, padding_mode='reflect'),
                RMSNorm(1, n_embd, 4),
                nn.SiLU(),
            )
...
class DoubleStreamRWKVBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.MIFM = MultiModalityFusion(n_embd, MIFM_modal_chan, MIFM_mask_chan, MIFM_llm_chan, 
                                        ...
                                        last_enc_block=last_enc_block, shift_type=shift_type, 
                                        n_layer=n_layer, add_mm_tokens=add_mm_tokens)
        self.MIFM_lerp_ratio = nn.Parameter(torch.zeros(n_embd))
        ############# BRWKV #############
        assert scan_mode in [
            "K0",
            "K2",
        ...
        self.modulator1 = Modulator(dim_att, double=False)
        self.modulator2 = Modulator(dim_ffn, double=False)
        ...
        self.only_img_attn_ln = RMSNorm(1, dim_att, 3)
        self.fusion_img_llm_ln = RMSNorm(1, dim_ffn, 3)
        # if self.has_llm:
    ...
    def BRWKV_img_forward(self, 
        ...
        x = x.lerp(MIFM_img, self.MIFM_lerp_ratio.view(1, -1, 1, 1))
...
class SingleStreamRWKVBlock(nn.Module):
    ...
    def forward(self, 
        ...
        x = x.lerp(MIFM_feat, self.MIFM_lerp_ratio.view(1, -1, 1, 1))
...
class FusionSequential(nn.Module):
    def __init__(self, 
        ...
        if resi_connection == '1conv':
            ...
            r_dim = dim // 2
            self.conv = nn.Sequential(nn.Conv2d(dim, r_dim, 3, 1, 1, padding_mode='reflect'), 
                                      nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                      nn.Conv2d(r_dim, r_dim, 1, 1, 0, padding_mode='reflect'),
                                      nn.LeakyReLU(negative_slope=0.2, inplace=True),
                                      nn.Conv2d(r_dim, dim, 3, 1, 1, padding_mode='reflect'))
        else:
            # to save parameters and memory
            # used in Swin
...
class UNetDownsample(nn.Module):
    def __init__(self,
        ...
        if down_type == "conv":
            self.down = nn.Conv2d(chan, chan * chan_r, r, r, bias=False, padding_mode='reflect')
        else:
            raise NotImplementedError(f"down type {down_type} not implemented")
...
class RWKVFusion(BaseModel):
    ...
    def __init__(
        ...
        layer_resi_connection: str="1conv",
        shift_type: str="none",               # shift for MEF, MFF will make the feature distorted
        ############### Fusion Config ###############
        upscale: int=1,
...
if __name__ == "__main__":
    ...
    class Tester:
        def __init__(self,
            ...
            cfg_mef_tiny = dict(
                    ...
                    ffn_groups=[2, 2],
                    mid_window_size=16,
                    enc_window_sizes=[32,32],#[14, 14, 14],
                    dec_window_sizes=[32,32],#[14, 14, 14],
                    ffn_hidden_rates=[2, 2],
        ...
        def test_forward_backward(self, real_img=False):
            img, cond, mask, llm_feature = self.prepare_data(real_img)
            out = self.net._forward_implem(img, cond, mask, llm_feature)
    ...
    tester = Tester(1, (64, 64), 'pan', 'small', 
    ...
    # tester.test_forward()
    tester.test_forward_backward(real_img=False)
