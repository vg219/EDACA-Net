...
class ConditionalNAFNet(BaseModel):
    ...
    def _forward_implem(self, inp, cond, c_shuffle=False):
        ...
        for i, (encoder, down) in enumerate(zip(self.lemm_encoders, self.lemm_downs)):
            ...
            x, states = encoder.LEVM_enc_forward(x, cond, c_shuffle, states)
            lemm_encs.append(x)
        ...
        x, states = self.middle_blks.LEVM_enc_forward(x, cond, c_shuffle, states)
        ...
        for i, (decoder, up, skip_scale, enc_skip, enc_state_skip) in enumerate(zip(self.lemm_decoders, self.lemm_ups, self.skip_scales,
            ...
            x, states = decoder.LEVM_dec_forward(x, cond, c_shuffle, states, enc_state_skip)
...
if __name__ == "__main__":
    ...
    for img_sz in [64]:
        ...
        chan = 8
        pan_chan = 1
        ms = torch.randn(1, chan, ms_img_sz, ms_img_sz).to(device)#.bfloat16()
        img = torch.randn(1, chan, ms_img_sz * scale, ms_img_sz * scale).to(device)#.bfloat16()
        cond = torch.randn(1, pan_chan, ms_img_sz * scale, ms_img_sz * scale).to(device)#.bfloat16()
        gt = torch.randn(1, chan, ms_img_sz * scale, ms_img_sz * scale).to(device)#.bfloat16()
        ...
        #             # [B, K, D, HW*HW]
        #             print(ori_xs.shape, updated_xs.shape)