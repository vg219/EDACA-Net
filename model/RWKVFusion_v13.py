...
from model.module.rwkv_v6_multi_modal import VRWKV_ChannelMix_wkv5 as RWKV_CMix
from model.module.rwkv_v6_multi_modal import VRWKV_SpatialMix_wkv5 as RWKV_TMix
...
logger = easy_logger(func_name='RWKVFusion_v13', level='INFO')
...
class Modulator(nn.Module):
    def __init__(self, dim: int, double: bool=True, K: int=1, c_first: bool=True):
        super().__init__()
        self.double = double    
        self.multip = 6 if double else 3
        self.K = K
        self.chunk_dim = 1 if c_first else -1
        
        if c_first:
            self.modulated = nn.Conv2d(dim, dim * self.multip, 1, bias=False)
        else:
    ...
    def forward(self, x):
        ...
        return self.modulated(F.silu(x)).chunk(self.multip, dim=self.chunk_dim)
...
class MultiModalityFusion(nn.Module):
    ...
    def __init__(self,
        ...
        self.txt_cat_order = 0  # scan_id % 2
        ...
        if mask_chan is not None:
            ...
            self.mask_convs = nn.Sequential(
                ...
                nn.Conv2d(n_embd // mask_ratio, n_embd, kernel_size=3, padding=1, 
                          groups=n_embd // mask_ratio, bias=False, padding_mode=CONV_PAD_MODE),
                norm_cls_channel_first(n_embd),
                nn.SiLU(),
            )
            
        # img and txt out norm
        self.mm_img_ln = norm_cls_channel_first(n_embd)
        ...
        if llm_chan is not None:
            ...
            self.multi_modal_attn = RWKV_TMix(
                ...
                scan_K=K,
                with_txt=llm_chan is not None,
            )
            self.multi_modal_img_ln = norm_cls_channel_first(n_embd)
    ...
    def forward(self, img_feat: torch.Tensor, cond_input: ConditionInput):
        ...
        if llm_feat is not None and self.with_llm_feat:
            ...
            # merge ESS
            img_feat_out = rearrange(img_feat_out, "(b k) (h w) d -> b k d h w", k=self.K, h=H, w=W)
            img_feat_out = self.merge(img_feat_out) / self.K
        ...
        img_feat = self.mm_img_ln(img_feat)
...
class DoubleStreamRWKVBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.mm_proj_in = nn.Conv2d(n_embd * 2, n_embd, 1, bias=False)
        ...
        self.modulator1 = Modulator(dim_att, double=False, K=K, c_first=True)
        self.modulator2 = Modulator(dim_ffn, double=False, K=1, c_first=False)
        ...
        self.only_img_attn_ln = norm_cls_channel_first(dim_att)
    ...
    def BRWKV_img_forward(self, 
        ...
        # x = x + MIFM_img
        # x = x.lerp(MIFM_img, self.lerp_factor.view(1, -1, 1, 1))
        # x = self.mm_proj_in(x)
        
        MIFM_img = MIFM_img.mul(self.lerp_factor.view(1, -1, 1, 1))
        x = self.mm_proj_in(torch.cat([x, MIFM_img], dim=1))
        ...
        x = ga1.view(B, C, -1).transpose(1, 2) * x_attn + x.view(B, -1, H * W).transpose(1, 2)
        ...
        prenorm_x = self.fusion_img_llm_ln(x)
...
class SingleStreamRWKVBlock(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.attn_modulator = Modulator(dim_att, double=False, c_first=True)
        self.ffn_modulator = Modulator(dim_ffn, double=False, c_first=False)
        ...
        self.x_ln1 = norm_cls_channel_first(dim_att)
    ...
    def forward(self, 
        ...
        ## MIFM
        MIFM_feat, *_ = self.MIFM(x, cond)
        # x = x.lerp(MIFM_feat, self.lerp_factor.view(1, -1, 1, 1))
        MIFM_feat = MIFM_feat.mul(self.lerp_factor.view(1, -1, 1, 1))
        x = self.mm_proj_in(torch.cat([x, MIFM_feat], dim=1))
        ...
        x_attn = self.drop_path(self.att(prenorm_x, None, (H, W)))
        x = ga1.view(B, C, -1).transpose(1, 2) * x_attn + x.view(B, -1, H * W).transpose(1, 2)
...
class FusionSequential(nn.Module):
    ...
    def dec_forward(self, feat, cond, patch_resolution) -> torch.Tensor:
        ...
        if self.resi_connection is not "none":
            # swinir type conv residual connection
            outp = outp.contiguous()
            outp = self.conv(outp)
            outp = outp + feat
    ...
    def forward(self, x):
        if self.up_type == "upsample":
            ...
            return F.interpolate(x,
                                 ...
                                 antialias=self.antialias)  # smooth upsample
        elif self.up_type == "transpose":
...
class RWKVFusion(BaseModel):
    ...
    def __init__(
        ...
        layer_resi_connection: str="none",
        ...
        ## fusion head
        self.fusion_head = nn.Conv2d(width, out_channel, 3, 1, 1, bias=False, padding_mode=CONV_PAD_MODE)
        ...
        for dec_layer_id, num in enumerate(reversed(dec_blk_nums)):
            ...
            self.skip_scales.append(
                nn.Parameter(torch.full((chan,), 2).sqrt(), requires_grad=True)
            )
        ...
        self.interpolate = lambda x, H, W: F.interpolate(x, (H, W), mode='bilinear', align_corners=False)
...
if __name__ == "__main__":
    ...
    class Tester:
        def __init__(self,
            ...
            self.channel_img = {'pan': 102, 'vif': 3, 'mef': 1}[task]
        ...
        def prepare_data(self, real_img=False):
            ...
            if real_img:
                ...
                cond = F.interpolate(cond, self.img_size, mode='bilinear', align_corners=False)
                # rgb fn
            ...
            if self.with_mask:
                if real_img:
                    mask = read_image('/Data3/cao/ZiHanCao/datasets/MEF-MEFB/mask/AirBellowsGap.png').to(device)
            ...
            if self.with_llm_feature:
                if real_img:
                    ...
                    llm_file = safe_open('/Data3/cao/ZiHanCao/datasets/MEF-MEFB/t5_feature_MEF-MEFB.safetensors', framework='pt')
                    llm_feature = llm_file.get_tensor('AirBellowsGap').to(device, dtype=self.dtype)
                else:
        ...
        def measure_throughput(self):
            ...
            throghtput = measure_throughput(self.net, [(self.channel_img, self.img_size[0], self.img_size[1]),
                                            ...
                                            32, num_warmup=10, num_iterations=50)
    ...
    tester = Tester(1, (256, 256), 'mef', 'tiny', 
                    with_mask=True, with_llm_feature=True,
                    device=device, dtype=torch.float32)
                    # pt_path='/Data3/cao/ZiHanCao/exps/panformer/log_file/RWKVFusion_v12_RWKVFusion/sice+mefb/2024-10-23-20-26-55_panRWKV_lka4t887_sice+mefb dataset with prior in weighted loss (w ssim loss) grad on color (K=0) fusion head no_roll in network/weights/ema_model.pth/model.safetensors')
