...
class MGFF(BaseModel):
    ...
    def fusion_train_step(self, vi, ir, gt, fusion_criterion, **_kwargs):
        fused = self._forward_implem(vi, ir)
        loss = fusion_criterion(fused, gt)
        
        return fused, loss
    
    @torch.no_grad()
    def fusion_val_step(self, vi, ir, *, patch_merge=False, **_kwargs):
        try:
            if not patch_merge:
                fused = self._forward_implem(vi, ir)
            else:
                pm_fn = PatchMergeModule(patch_merge_step=self._forward_implem,
                                ...
                                scale=1,
                                crop_batch_size=8)
                fused = pm_fn.forward_chop(vi, ir)[0]
        except Exception:
...
if __name__ == "__main__":
    ...
    # print(
    #     flop_count_table(
    #         FlopCountAnalysis(model, (torch.randn(1, 3, 256, 256).cuda(), torch.randn(1, 1, 256, 256).cuda()))
    #     )
    # )
    
    from utils import measure_throughput
    
    measure_throughput(model, ((3, 256, 256), (1, 256, 256)), 8, 10, 50)
