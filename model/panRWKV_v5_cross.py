...
from model.module.rwkv_v3 import (
    CrossScanTriton,
    CrossMergeTriton,
    CrossScan,
    CrossMerge,
...
from model.module.rwkv_v3 import VRWKV_ChannelMix as RWKV_CMix
from model.module.rwkv_v3 import VRWKV_SpatialMix_V6 as RWKV_TMix
...
class RWKVBlock_cross(nn.Module):
    def __init__(
        self,
        layer_id,
        n_layer=8,
        ...
        *,
        ...
        logger.info(
            f"layer_id: {layer_id}, scan_mode: {scan_mode}, scan_id: {scan_id}, out_norm: {not no_out_norm}"
        )
        ...
        self.x_convs = nn.Sequential(
            ...
            nn.ReLU(),
            nn.Conv2d(n_embd, n_embd, 1),
        )
        self.cond_convs = nn.Sequential(
            nn.Conv2d(cond_chan, n_embd, kernel_size=1),
            nn.Conv2d(n_embd, n_embd, kernel_size=3, padding=1, groups=n_embd, bias=False),
        )
        ...
        self.att = RWKV_TMix(dim_att, N_HEAD, n_layer, layer_id, shift_mode=shift_type, n_groups=4, with_cp=with_checkpoint)

        self.ffn = RWKV_CMix(dim_ffn, N_HEAD, n_layer, layer_id, 
                             shift_mode=shift_type, hidden_rate=1, n_groups=4, with_cp=with_checkpoint)

        self.x_ln1 = nn.GroupNorm(1, n_embd)  # LayerNorm(n_embd, 'BiasFree')
        self.x_ln2 = nn.GroupNorm(1, n_embd)  # LayerNorm(n_embd, 'BiasFree')
        self.x_ln3 = nn.GroupNorm(1, n_embd)  # LayerNorm(n_embd, 'BiasFree')
        ...
        if self.layer_scale:
            self.gamma1 = nn.Parameter(init_values * torch.ones((1, n_embd, 1)), requires_grad=True)
            self.gamma2 = nn.Parameter(init_values * torch.ones((1, n_embd, 1)), requires_grad=True)

    def forward(
        self, x: torch.Tensor, cond: torch.Tensor, patch_resolution=None
    ):
        b, c, h, w = x.shape
        inp = x
        T = h * w
        ...
        cond = self.scan(cond).view(b, -1, h * w)
        if self.layer_scale:
            # spatial mixing
            x = self.drop_path(self.gamma1 * self.att(self.x_ln1(x), self.x_ln3(cond), patch_resolution)) + x
            # Channel mixing
            x = self.drop_path(self.gamma2 * self.ffn(self.x_ln2(x), patch_resolution)) + x
        else:
            # spatial mixing
            x = self.drop_path(self.att(self.x_ln1(x), self.x_ln3(cond), patch_resolution)) + x
            # Channel mixing
...
@register_model("panRWKV_cross_v5")
class RWKVCrossFusion(BaseModel):
    def __init__(
        self,
        img_channel=3,
        condition_channel=9,
        ...
        self.patch_embd = nn.Conv2d(in_channels=img_channel + condition_channel,
        ...
        self.proj_out = nn.Conv2d(in_channels=width, out_channels=out_channel, kernel_size=1,
                                stride=1, bias=True)
    ...
    def _forward_implem(self, inp, cond):
        ...
        *_, H, W = x.shape
        ...
        for encoder, down in zip(self.encoders, self.downs):
            cond1 = F.interpolate(u_cond, (H, W), mode="bilinear", align_corners=False, antialias=True)
        ...
        cond2 = F.interpolate(u_cond, (H, W), mode="bilinear", align_corners=False, antialias=True)
        ...
        for decoder, up, enc_skip, skip_scale in zip(
            ...
            H = H * 2
            W = W * 2
            cond3 = F.interpolate(u_cond, (H, W), mode="bilinear", align_corners=False, antialias=True)
