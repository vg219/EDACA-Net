...
class Merge(BaseModel):
    def __init__(self, img_size=5, patch_size=1, in_chans1=3, in_chans2=31, embed_dim=48, num_heads1=[8, 8, 8],window_size=8, group=8, mlp_ratio=4.,dim=32,
                 num_heads2=[8, 8, 8], ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias', bias=False
                 ):

        super(Merge, self).__init__()
        self.direction1 = Direction1(img_size=img_size, patch_size=patch_size, in_chans=in_chans1+in_chans2, embed_dim=embed_dim,
    ...
    def _forward_implem(self, pan, lms,ms):
        xt = torch.cat((pan, lms), 1)
        D1_stage1, D1_stage2, D1_stage3 = self.direction1(xt)
        D2_stage3, D2_stage2, D2_stage1 = self.direction2(ms)
...
if __name__ == '__main__':
    ...
    sz = 64
    input1 = torch.randn(1, 3, sz, sz).cuda()
    input2 = torch.randn(1, 128, sz // 4, sz // 4).cuda()
    lms = torch.randn(1, 128, sz, sz).cuda()
    gt = torch.randn(1, 128, sz, sz).cuda()

    
    net = Merge(img_size=sz, patch_size=1, in_chans1=3, in_chans2=128, embed_dim=48, num_heads1=[8, 8, 8],
                       window_size=8, group=8,  mlp_ratio=4.,dim=48, num_heads2=[8, 8, 8],  ffn_expansion_factor=2.66,  LayerNorm_type = 'WithBias',
                       bias=False).cuda()
    criterion = torch.nn.L1Loss()
    sr,loss= net.train_step(input2, lms, input1, gt, criterion)
    # loss = criterion(sr, gt) 
    # loss[0].backward()2
    print(sr.shape)
    ...
    print(flop_count_table(FlopCountAnalysis(net, (input1, lms,input2))))


### pavia     3.056598M              | 3.569002G     
### chikusei  3.263122M              | 4.371793G     
