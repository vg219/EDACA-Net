...
from utils.general import (
    ...
    colorstr,
    print_args,
    coco80_to_m3fd_class,
    suppress_non_m3fd_in_coco_classes,
)
...
def run(
    ...
    for batch_i, (img, targets, paths, shapes) in enumerate(dataloader):
        ...
        # maps to m3fd classes
        # targets = coco80_to_m3fd_class(targets)
        ...
        ## remove other classes output
        # M3FD dataset
        # m3fd_in_coco_class = torch.tensor([0, 2, 5, 9, 3, 7]).to(device)
        # out_m3fd_box = out[:, :, :5]
        # out_m3fd_class = out[:, :, m3fd_in_coco_class + 5]
        # out = torch.cat([out_m3fd_box, out_m3fd_class], dim=2)
        
        # MSRS dataset
        msrs_in_coco_class = torch.tensor([0, 2]).to(device)
        out_msrs_box = out[:, :, :5]
        out_msrs_class = out[:, :, msrs_in_coco_class + 5]
        out = torch.cat([out_msrs_box, out_msrs_class], dim=2)
        ...
        for si, pred in enumerate(out):
            # pred = suppress_non_m3fd_in_coco_classes(pred)
        ...
        if plots and batch_i < 3:
            ...
            print(f"saving predicted image at {f}")
    ...
    # Print results
    result_txt = open(save_dir / 'results.txt', 'w')
    pf = "%20s" + "%11i" * 2 + "%11.3g" * 4  # print format
    head = '%20s%11s%11s%11s%11s%11s%11s' % ("Class", "Images", "Labels", "P", "R", "mAP@.5", "mAP@.5:.95")
    msg = pf % ("all", seen, nt.sum(), mp, mr, map50, map)
    print(head + "\n" + msg)
    result_txt.write(head + "\n" + msg)

    # Print results per class
    if (verbose or (nc < 50 and not training)) and nc > 1 and len(stats):
        for i, c in enumerate(ap_class):
            per_cls_msg = pf % (names[c], seen, nt[c], p[i], r[i], ap50[i], ap[i])
            print(per_cls_msg)
            result_txt.write(per_cls_msg)
