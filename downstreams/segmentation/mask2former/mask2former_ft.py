...
# modified by huggingface mask2former fintune tutorial


def create_msrs_seg_hf_dataset():
    # create dataset
...
def parse_args():
    ...
    parser.add_argument("--num_train_epochs", type=int, default=800, help="Total number of training epochs to perform.")
    parser.add_argument(
        "--max_train_steps",
        type=int,
        default=None,
    ...
    parser.add_argument("--output_dir",
                        type=str,
                        default='/Data3/cao/ZiHanCao/exps/panformer/downstreams/segmentation/runs', 
...
def augment_and_transform_batch(
    ...
    for pil_image, pil_annotation in zip(examples["image"], examples["annotation"]):
        ...
        pil_annotation = np.array(pil_annotation)#[..., np.newaxis].repeat(2, axis=-1)
...
def evaluation_loop_instance(model, image_processor, accelerator: Accelerator, dataloader, id2label):
...
@torch.no_grad()
def evaluation_loop_semantic(model,
                             image_processor: AutoImageProcessor, 
    ...
    if args.do_reduce_labels:
        n_class -= 1  # remove background class
    
    if miou_method == 'torchmetrics':
        logger.info('computing miou using torchmetrics')
        metric = MeanIoU(num_classes=n_class, per_class=True, include_background=args.do_reduce_labels)
    else:
        logger.warning('the huggingface implememted miou is extremly slow')
        metric = evaluate.load("mean_iou", num_labels=len(id2label), trust_remote_code=True)
    
    
    metrics: list[torch.Tensor] = []
    n_bs: list[int] = []
    
    for bi, inputs in tqdm(enumerate(dataloader, 1), total=len(dataloader), disable=not accelerator.is_local_main_process, leave=True):
        with torch.no_grad():
            outputs = model(**inputs)
        ...
        def to_seg_map(shape: str, outputs, target_sizes, n_classes):
            ...
            if shape == 'hw':
                ...
                for i in range(len(seg_map)):
                    seg_map[i] = torch.nn.functional.one_hot(seg_map[i].long(), num_classes=n_classes).permute(2, 0, 1)
                
            return seg_map
        
        _seg_map_shape = 'chw' if miou_method == 'torchmetrics' else 'hw'
        seg_map = to_seg_map(_seg_map_shape, outputs, target_sizes, n_classes=n_class)

        for target, class_label, output in zip(inputs["mask_labels"], inputs["class_labels"], seg_map):  # target[1]
            # Update metric for batch targets and predictions
            if miou_method == 'hf_evaluate':
                ...
                labels = torch.zeros(n_class, *target.shape[-2:], dtype=torch.long, device=target.device)
    ...
    if miou_method == 'torchmetrics':
        ...
        metrics = metric.compute(num_labels=len(id2label), ignore_index=0, reduce_labels=image_processor.do_reduce_labels)
...
def main():
    ...
    dataset = create_msrs_seg_hf_dataset()
    ...
    id2label = json.load(open('/Data3/cao/ZiHanCao/datasets/MSRS/MSRS-id2label.json', 'r'))
    ...
    for epoch in range(starting_epoch, args.num_train_epochs):
        ...
        # metrics = evaluation_loop_semantic(model, image_processor, accelerator, valid_dataloader, id2label, args=args)
