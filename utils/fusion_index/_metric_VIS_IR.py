...
def Qabf_getArray_torch(img: torch.Tensor):
    ...
    aA = torch.where(SAx == 0, 
                     torch.tensor(math.pi / 2, device=SAx.device, dtype=SAx.dtype), 
                     torch.atan(
                            torch.where(SAx != 0, SAy / SAx, 
                                        torch.tensor(0., device=SAx.device, dtype=SAx.dtype))
                         )
                     )
...
if __name__ == "__main__":
    ...
    print(timeit(lambda: evaluate_VIF_metric_numpy(fused.cpu().numpy(), ir.cpu().numpy(), vi.cpu().numpy(), "all"), number=10))
