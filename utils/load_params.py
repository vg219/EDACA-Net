...
def module_load(path, 
    ...
    if path.endswith('pth') or path.endswith('pt') or path.endswith('bin'):
        ...
        assert osp.isdir(path), f'{path} should be a directory to load deepspeed checkpoint'
        load_engine = lambda path, **_kwargs: load_state_dict_from_zero_checkpoint(model, path)
        ...
        if not isinstance(params_load, (list, tuple)):
            ...
            for (s_name, s_param), (name, param) in zip(param_load_ziped, model.named_parameters()):
                ...
                if saved_shape != required_shape:
                    if strict:
                        logger.warning(f'param shape unmatched, {name} requires: {required_shape}, but got {s_name}: {saved_shape}')
                        if not full_unmatched_log:
                            logger.error('model load failed! shape of params does not match!')
            ...
            for s_param, param in zip(params_load, model.parameters()):
                ...
                if saved_shape != required_shape:
                    if strict:
                        logger.warning(f'param shape unmatched, requires: {required_shape}, but got {saved_shape}')
                        if not full_unmatched_log:
                            logger.error('model load failed! shape of params does not match')
                            raise RuntimeError('model load failed! shape of params does not match!')
                        else:
                        ...
                        logger.warning(f'skip the shape mismatched param, current shape {required_shape} but loaded shape {saved_shape}')
                        continue
                param.data.copy_(s_param.data)
