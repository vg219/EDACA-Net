...
class BestMetricSaveChecker:
    ...
    def __init__(self,
        ...
        self.avg_metrics = isinstance(self.metric_name, (tuple, list)) and avg_metrics_weights is not None
    ...
    def _default_setting(self):
        assert isinstance(self.metric_name, str), 'default setting only support single metric name'
    ...
    def __call__(self, val_metrics: dict[str, float], *args):
        ...
        if self.verbose:
            if _save:
                logger.info(f'best metric {self.metric_name} (weighted or one item), \n\t\t' +
