...
def easy_logger(level: str | None='INFO', 
    ...
    level = level.upper()
    if os.getenv("LOGGER_LEVEL") is None:
    ...
    if file_name and is_main_process() and not never_save_in_file:
        ...
        file_handler = RichHandler(console=file_console, show_path=False, level=level, markup=True)
    ...
    class FuncNameFilter(logging.Filter):
        def filter(self, record):
            ...
            record.func_name = f"[u]{func_name}[/u]"
            return True
    ...
    class ProtocalLogger(Protocol):
        ...
        @classmethod
        def has_file_handler(cls):
            # logger: logging.Logger
            return any([isinstance(hdl, (logging.FileHandler, RichHandler)) for hdl in logger.handlers])
        
        @classmethod
        def is_file_handler_set(cls):
            return cls._file_handler_set
                
        @classmethod
        def may_set_file_handler(cls):
            if cls.is_file_handler_set() or cls._not_save_in_file:
                return
            else:
                file_name = os.environ.get('LOG_FILE', None)
                # has file_handler or not
        ...
        def print(ctx, *msg, sep=" ", level: "str | int"="INFO"):
            ...
            ctx.may_set_file_handler()
            logger.log(level, msg, extra={"markup": True})
            
        @classmethod
        def debug(ctx, *msg, sep=" ", level: "str | int"="INFO"):
            pass
        
        @classmethod
        def info(ctx, *msg, sep=" ", level: "str | int"="INFO"):
            pass
        
        @classmethod
        def warning(ctx, *msg, sep=" ", level: "str | int"="INFO"):
            pass
    ...
    # add console attributes to logger
...
def get_logger(
    ...
    **_kwargs,  # to avoid unused kwargs
    ...
    if base_path is not None:
        ...
        for n, level in zip(file_handler_names, file_level):
            if method_dataset_as_prepos:
                if 'full_arch' not in args:
                    full_arch = 'default_run'
                else:
                    full_arch = args.full_arch
                file_log_dir = os.path.join(base_path, full_arch, args.dataset, name)
            else:
...
class TensorboardLogger:
    def __init__(
        ...
        args: "NameSpace"=None,
        tsb_logdir=None,
        tsb_comment=None,  # only to Tensorboard comment
        ...
        if args.default_none_getattr('run_id') is not None:
            ...
            assert args.default_none_getattr('comment') is not None, "`args.comment` should not be None"
            args.logger_config.name += f"_{args.comment}"
    ...
    def log_images(self, batch_imgs: Sequence, nrow: int, names: Sequence, task: str,
                   epoch: int, ds_name: str | None=None, **grid_kwargs):
