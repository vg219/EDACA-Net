...
class AnalysisPanAcc(object):
    def __init__(self, ratio=4, ref=True, ergas_ratio: int = 4, **unref_factory_kwargs):
        ...
        if ratio is None: 
            ratio = ergas_ratio
            warn('`ergas_ratio` is deprecated, use ratio instead', category=DeprecationWarning)
        self.ratio = ratio
        self.ref = ref
        ...
        if ref:
            ...
            if self.default_max_value is None: 
                _default_max_value = {'QB': 2047, 'IKONOS': 1023, 'WV2': 2047, 'WV3': 2047, 
                                      ...
                                      'CAVE_X4': 1, 'CAVE_X8': 1, 'HARVARD_X4': 1, 'HARVARD_X8': 1,
                                      'GF5':1, 'GF2-GF5': 1,}
                self.default_max_value = _default_max_value.get(sensor)
                logger.warning(f'>>> `default_max_value` is not specified, set it according to `sensor`:'
